...
"""Attribute a recorded policy violation to a document.
    ...
    python scripts/investigate.py [--live | --offline]
...
is the culprit; the finding is appended to the episode chain as a signed
attribution record and the chain is re-anchored.
"""
...
from record_episode import FORBIDDEN_TOOLS, QUERY  # noqa: E402
...
def main(argv: list[str] | None = None) -> int:
    ...
    anchor_key = SigningKey.generate(args.anchor_id)
    ...
    print(f"record     seq {investigation.record['seq']}, signed by "
          f"{investigation.record['signature']['key_id']}")
    print(f"{args.anchor_id}   re-anchored over the extended chain")
